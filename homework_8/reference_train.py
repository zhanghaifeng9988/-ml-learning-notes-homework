...
# NORMALIZE 是标准化：把像素值从 [0,1] 变成均值 0、标准差 1 附近。
# 用 ImageNet 的统计量是因为很多预训练模型都这么干，属于惯例。

def seed_everything(seed: int = SEED) -> None:
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)      # torch CPU/GPU 随机
    torch.use_deterministic_algorithms(True)   # 强制用确定性算法
    torch.backends.cudnn.deterministic = True    # cudnn 确定性
    torch.backends.cudnn.benchmark = False       # 关掉自动调优


class HairModel(nn.Module):
    def __init__(self):
        super().__init__() #输入 3 通道（RGB），输出 32 通道，卷积核 3×3，步长 1，不补边。
        self.conv = nn.Conv2d(3, 32, kernel_size=3, stride=1, padding=0)
        self.pool = nn.MaxPool2d(kernel_size=2) # 2×2 最大池化，尺寸减半：198 → 99。
        self.hidden = nn.Linear(32 * 99 * 99, 64) #展平后是 32×99×99 个数，接到 64 维隐藏层。
        self.output = nn.Linear(64, 1) # 输出 1 个数（logit），用于二分类。

# 
    def forward(self, x):
        x = self.pool(nn.functional.relu(self.conv(x)))  # 卷积→ReLU→池化
        x = x.flatten(start_dim=1)                       # 展平（保留 batch 维）
        x = nn.functional.relu(self.hidden(x))           # 全连接→ReLU
        return self.output(x)                            # 输出 logit
...
def augmented_train_transform():
    return transforms.Compose([
        transforms.Resize(  # 先统一尺寸
            (200, 200),
            interpolation=transforms.InterpolationMode.BILINEAR,
        ),
        transforms.RandomRotation( # 随机旋转 ±50 度
            50,
            interpolation=transforms.InterpolationMode.NEAREST,
        ),
        transforms.RandomResizedCrop( # 随机裁剪再缩放
            200,
            scale=(0.9, 1.0),
            ratio=(0.9, 1.1),
            interpolation=transforms.InterpolationMode.BILINEAR,
